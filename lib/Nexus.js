    ...
    getUserInfo(oauthToken) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (oauthToken === undefined) {
                response = yield this.request(`${param.USER_SERVICE_API_URL}/oauth/userinfo`, this.args({}));
            }
            else {
                const result = Object.assign({}, this.mBaseData);
                result.headers['Authorization'] = `Bearer: ${oauthToken}`;
                response = yield this.request(`${param.USER_SERVICE_API_URL}/oauth/userinfo`, result);
            }
