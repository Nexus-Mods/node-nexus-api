...
function getTextFromStream(readableStream_1) {
    return __awaiter(this, arguments, void 0, function* (readableStream, encoding = 'utf8') {
        let reader = readableStream.getReader();
        let utf8Decoder = new util_1.TextDecoder(encoding);
        let nextChunk;
        let resultStr = '';
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        for (const [name, value] of formData) {
            if (value instanceof File) {
                data.append(name, yield getTextFromStream(value.stream()));
            }
            else {
...
function createStreamableFile(path) {
    return __awaiter(this, void 0, void 0, function* () {
        const name = (0, path_1.basename)(path);
        const handle = yield fs.promises.open(path);
...
function handleRestResult(resolve, reject, url, error, response, body, onUpdateLimit) {
    ...
    if (error !== null) {
        try {
            ...
            if (message) {
                ...
                return reject(new customErrors_1.NexusError(translateMessage(message), response.status, url, message, data.error_description));
            }
        }
        catch (_e) {
        }
    ...
    try {
        ...
        if ((response.status === 521)
            || (body === 'Bad Gateway')) {
            return reject(new customErrors_1.NexusError('API currently offline', response.status, url, body));
        }
        ...
        if ((response.status < 200) || (response.status >= 300)) {
            ...
            return reject(new customErrors_1.NexusError(message, response.status, url, message));
        }
        ...
        if ((body.length > 0) && (body[0] === '<')) {
            return reject(new customErrors_1.NexusError('API currently not reachable, please try again later.', response.status, url, 'API_UNREACHABLE'));
        }
...
function restGet(inputUrl, args, onUpdateLimit) {
    return new Promise((resolve, reject) => {
        ...
        fetch(url.URL.parse(finalURL), {
            ...
            if (err !== undefined) {
                return reject(new customErrors_1.HTTPError(status, err, '', finalURL));
            }
        ...
        fetch(url.URL.parse(finalURL), {
            ...
            getTextFromStream(res.body, 'utf8').then((rawData) => {
                ...
                if (![200, 201].includes(status)) {
                    err = new customErrors_1.HTTPError(res.status, res.statusText, rawData, finalURL);
                }
...
class Nexus {
    ...
    getCollectionGraph(query_1, slug_1) {
        return __awaiter(this, arguments, void 0, function* (query, slug, ignoreAdultBlock = true) {
            yield this.mQuota.wait();
    ...
    getCollectionRevisionGraph(query_1, collectionSlug_1, revisionNumber_1) {
        return __awaiter(this, arguments, void 0, function* (query, collectionSlug, revisionNumber, ignoreAdultBlock = true) {
            yield this.mQuota.wait();
            const parameters = {
                slug: { type: 'String', optional: false },
                viewAdultContent: { type: 'Boolean', optional: true },
            ...
            return this.checkFileSize(fileBundle)
                ...
                (() => __awaiter(this, void 0, void 0, function* () {
                    ...
                    if (fileBundle !== undefined) {
                        form.append('feedback_file', yield createStreamableFile(fileBundle));
                    }
                    ...
                    fetch(url.URL.parse(inputUrl), {
                        method: 'POST',
                        headers,
                        signal,
                        body: yield formatFormData(form),
                    }).then((res) => {
                        clearTimeout(timeoutId);
                        getTextFromStream(res.body, 'utf8').then((rawData) => {
                            if (res.status >= 400) {
                                return reject(new customErrors_1.HTTPError(res.status, res.statusText, rawData, inputUrl));
                            }
                            else {
